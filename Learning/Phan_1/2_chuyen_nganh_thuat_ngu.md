...
* Phương pháp phát hiện:
  * Quy tắc IQR (Interquartile Range - Khoảng tứ phân vị):
    * Tính khoảng cách giữa phân vị 75% (Q3) và phân vị 25% (Q1): IQR = Q3 - Q1.
    * Điểm dữ liệu x được coi là Outlier nếu x < Q1 - 1.5 * IQR hoặc x > Q3 + 1.5 * IQR.
...
* Phương pháp phổ biến:
  * Label Encoding: Chuyển đổi các danh mục chữ thành các số nguyên tăng dần (0, 1, 2...). Chỉ nên dùng khi biến phân loại đó có tính thứ tự rõ ràng (ví dụ: Thấp -> 0, Trung bình -> 1, Khá -> 2).
...
* Ý nghĩa: Tạo thêm các đặc trưng bằng cách bình phương hoặc nhân chéo các thuộc tính có sẵn (x1^2, x1 * x2). Kỹ thuật này giúp các mô hình tuyến tính (như Linear Regression) có thể học được các đường ranh giới phi tuyến tính phức tạp.
